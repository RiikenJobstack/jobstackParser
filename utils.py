...
def parse_resume_temp(filename: str, content: bytes) -> dict:
    ...
    if filename is 'resume.txt':
        ...
        raw_text = extract_text_from_resume(filename, content)
    transform_result = transform_text_to_resume_data_temp(raw_text)
    structured_data = transform_result.get("analysis_content", {})
    # Cache the complete result
    _set_cache(cache_key, structured_data)
    return {
        "success": True,
        "structured_data": structured_data,
        "debug": {**transform_result.get("debug_info", {}), "stage": "full_parse"}
    }
...
def create_prompt_cache(static_instructions: str, model: str) -> str:
    ...
    try:
        ...
        endpoint = f"https://us-central1-aiplatform.googleapis.com/v1/projects/jobstack-ai-gemini-model/locations/us-central1/cachedContents"
        ...
        data = {
            "model": f"projects/jobstack-ai-gemini-model/locations/us-central1/publishers/google/models/{model}",
            "contents": [
                {
                    "role": "user",
...
def count_tokens(prompt: str, model: str) -> int:
    ...
    try:
        ...
        endpoint = f"https://us-central1-aiplatform.googleapis.com/v1/projects/jobstack-ai-gemini-model/locations/us-central1/publishers/google/models/{model}:countTokens"
